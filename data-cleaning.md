...
**Note:** This is an optional challenge if you want to practice using
`assertr`. If not, fell free to skip it to the next page where we will
talk about data manipulation with the `tidyverse`package.

**Challenge:** Use the `verify()` and `assert()` functions to write data
validation code that checks that:
